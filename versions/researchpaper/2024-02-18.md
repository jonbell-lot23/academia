...
1. **Help is better than no help**: Across the findings, we see that doing nothing to help someone understand context always tests worse than doing something. This is an unsurprising result, but leads to finding #2, with its inconvenient implications.

2. **Creating these features is extremely difficult:** In a perfect world, every word, paragraph, or full paper would always provide context that doesn't take you out of flow. But in the real world, every one of these features requires development, testing, upcoming, and training. The key here is not to build cool features, but to understand the tradeoffs inherent in creating and publishing content.
...
Across the findings, we see that doing nothing to help someone understand context always tests worse than doing something. This is an unsurprising result, but leads to finding #2, with its inconvenient implications.

### Creating these features is extremely difficult
In a perfect world, every word, paragraph, or full paper would always provide context that doesn't take you out of flow. But in the real world, every one of these features requires development, testing, upcoming, and training. The key here is not to build cool features, but to understand the tradeoffs inherent in creating and publishing content.

### We need better metrics to rate ROI
If a feature takes a year to develop, but only saves someone twenty seconds of time, it has poor ROI. The problem is that despite my efforts here, there's no clear way to quantify if a feature that adds helpful content could be worth it.

### Some things are simply not possible without augmented text
Many of the items compared boil down to a clear comparison between task completion across two flows. But some can't be compared because it's simply not possible to do without a change in technology. This is part of the tradeoff discussion.

### The possibilities are vast and contextual
I've carved off a small handful of examples to compare here, but any publication of content could come up with new features. For example, imagine tools that help understand memes, or completely different tools designed for lawyers or doctors to understand their complex fields.
...
**Context is King**
Using a summary of the findings, we stress the importance of context in software design and education.

**Experience is difficult to quantify**
We talk about how the raw numbers don't tell the story. For example, something might be 2 seconds faster, which doesn't sound like much. But 2 seconds can be an eternity when you're dealing with certain scenarios. Not only that, there are some things that aren't possible without augmented text.

**A Failure of Imagination**
How did we get here? This is where we should talk about how multimedia evolved to this point, where the most salient new tool for context is the humble hyperlink. But there are so many more things we can do if we think bigger than A/V and unwieldy apps. Tie this back into how augmented reality and mixed reality is providing something new, and text needs the same.

**Explorables was a Blind Alley**
Talk about how inspired I was by Explorable Explanations and the educational theory books that led to it. Give lots of examples of awesome context across lots of different interactive fields. But in the end, it's just another app. It's just more software. That helps when you want to be in a single app at a time. It doesn't help when your tasks traverse multiple scenarios and apps. This is where augmentation is necessary, not just plopping a/v + apps into your vision.

**Browsers and Hackers Aren't Going to Get Us There**
It's possible to augment text with specialised software, browser plugins, and so forth. The world of accessibility does this for a lot of things, and this can get us pretty far. But certain things need to be done at a global level. Customisation is great, but imagine if everyone had custom software for basic things like reading. That's not good. We need to make some things globally accessible.

**Don't Overlook Fragmentation**
A lot of advocates for this kind of thing hand-wave how difficult implementation is. The world is extremely fragmented, and always will be. Getting Apple and Microsoft to agree is hard. Getting Google to agree also is harder still. Adding Wordpress in there is insane. Any ideas for the future need to recognise that standards are hard.
